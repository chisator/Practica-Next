...
function formatear(totalPaidInvoices: number) {
  const amount = totalPaidInvoices / 100;
...
export default async function Page() {
  ...
  const numberOfCustomers= await fenumberOfCustomers();
  return (
    <main>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <Card
          title="Collected"
          value={formatear(totalPaidInvoices)}
          type="collected"
        />
        <Card
          title="Pending"
          value={formatear(totalPendingInvoices)}
          type="pending"
        />
